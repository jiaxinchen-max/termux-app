...
public class LorieView extends SurfaceView implements InputStub {
    ...
    private boolean hasLorieHost() {
        if (mLorieHost != null)
            return true;

        try {
            return getActivity() instanceof LorieViewRuntimeApi.LorieHost;
    ...
    private void updateViewport() {
        if (mCallback == null || !hasLorieHost())
            return;

        Prefs prefs = getLorieHost().getX11Prefs();
        int surfaceW = getMeasuredWidth();
        int surfaceH = getMeasuredHeight();
        int availableLeft = contentInsets.left;
        int availableTop = contentInsets.top;
        ...
        if (cursorLocker != null && cursorLocker.isEnabled()) {
            ...
            int minPanX = (int) Math.ceil(visibleCenterX - drawW - baseLeft);
            int maxPanX = (int) Math.floor(visibleCenterX - baseLeft);
            int minPanY = (int) Math.ceil(visibleCenterY - drawH - baseTop);
            int maxPanY = (int) Math.floor(visibleCenterY - baseTop);
...
//    }
    ...
    @CriticalNative
    static native boolean xConnected();
    @FastNative public static native boolean killExternalServer(int signal);
